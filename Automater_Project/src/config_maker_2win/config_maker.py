...
# updated val if the var_list var is in section list
def rewrite(line, var_list, config, section):
  s = line
  l_line = line.lstrip()
  for v in var_list:
    v_quote = "\""+v[1]+"\""
    l_line_s = l_line.lower()
    if v[0] in section and l_line_s.startswith(v_quote):
      val = config[v[0]][v[1]]
...
output_win2 = output_dir + r'\window2_' + str(s) + '.txt'
...
with open(default_cons, "r") as def_C:
  ...
  for line in lines:
    s = rewrite(line, var_list, config, ["Loads","Conditioning","Ventilation","DomHotWater","Materials"])
    out_C.writelines(s)
out_C.close()
...
with open(default_win, "r") as def_W:
  ...
  for line in lines:
    s = rewrite(line, var_list, config, ["Window"])
    out_W.writelines(s)
out_W.close()

# output win2
out_W2 = open(output_win2, "w")
with open(default_win, "r") as def_W:
  lines = def_W.readlines()
  for line in lines:
    s = rewrite(line, var_list, config, ["Window2"])
    out_W2.writelines(s)
out_W2.close()